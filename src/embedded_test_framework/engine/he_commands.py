        ...
        if not self.connected:
            if self._password is None:
                ...
                try:
                    ...
                    client.close()
                    raise OperationTimeout("SSH password connection timed out") from exc
        ...
        if self._client is not None:
            ...
            try:
                ...
                if transport is None or not transport.is_active():
                    self.connected = False
                    raise DeviceDisconnected("SSH session is disconnected")
                channel = transport.open_session(timeout=duration)
                ...
                self.connected = False
                raise DeviceDisconnected("SSH connection lost during command execution") from exc
            except TimeoutError as exc:
