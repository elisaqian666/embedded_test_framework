...
from ..libs.errors import CapabilityError, CleanupError, ConfigurationError, TransportError, DeviceDisconnected
from ..libs.contracts import CommandChannel, RequestChannel, ByteChannel, FileChannel
...
class Device:
    ...
    @property
    def engines(self):
        """Read-only engine inventory. Device retains connection and cleanup ownership."""
        return MappingProxyType(self._channels)

    def bind_capability(self, name, capability, *, replace=False):
        from ..dut.device_features import Capability
    ...
    @log_operation
    def reboot(self, timeout=60, wait_until_reboot=True, *, channel="shell", interval=1.0):
        """Reboot using a power adapter, or the legacy Linux shell workflow.

        Custom power capabilities retain ownership of their reboot workflow.
        The shell workflow waits for a new boot and optional health readiness.
        """
        if "power" in self._capabilities:
            options = {}
            if timeout != 60:
                options["timeout"] = timeout
            if wait_until_reboot is not True:
                options["wait_until_reboot"] = wait_until_reboot
            if interval != 1.0:
                options["interval"] = interval
            return self.capability("power").reboot(**options)
        from ..dut.linux_device import LinuxPowerCapability
        return LinuxPowerCapability(self, channel=channel).reboot(
            timeout=timeout, wait_until_reboot=wait_until_reboot, interval=interval)
    ...
    def get_ipv4_addresses(self, interface="eth0", *, channel="shell", command="ifconfig",
        ...
        from ..dut.linux_device import LinuxNetworkCapability
