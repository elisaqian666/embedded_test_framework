...
class SshEngine(object):
    ...
    def __re_init__(self):
        ...
        time.sleep(1)
        new_eng = make_ssh_engine(self.address, self.username, self.password, port=self.port)

        if self.ssh_con:  # And we still have the old SSH connection when there is issue calling "make_ssh_engine"
            try:
                self.ssh_con.close()
            except Exception:
    ...
    @deprecation.deprecated(
        deprecated_in="1.6.0",
        removed_in="1.7.0",
        details="sshengine.do_quick_command() is deprecated please use sshengine.do_command()",
    )
    def do_quick_command(self, command, time_out=60):
        return self.do_command(command, time_out=time_out)
    ...
    @deprecation.deprecated(
        deprecated_in="1.6.0",
        removed_in="1.7.0",
        details="sshengine.do_quick_command_w_exitstatus() is deprecated please use sshengine.do_command_w_exitstatus()",
    )
    def do_quick_command_w_exitstatus(self, command, time_out=60):
        return self.do_command_w_exitstatus(command, time_out=time_out)
    ...
    @deprecation.deprecated(
        deprecated_in="1.6.0",
        removed_in="1.7.0",
        details="sshengine.put_files_via_scp() is deprecated please use sshengine.put_files()",
    )
    def put_files_via_scp(self, source, destination):
        """
        sends a file via scp to a server"""
        self.put_files(source, destination)
    ...
    @deprecation.deprecated(
        deprecated_in="1.6.0",
        removed_in="1.7.0",
        details="sshengine.get_files_via_scp() is deprecated please use sshengine.get_files()",
    )
...
def make_ssh_engine(address: str, username: str, password: str, port: int = 22, retry: int = 4, timeout: float = 10) -> SshEngine:
    ...
    return SshEngine(connection, address, username, password, port)
