...
class ThreadedWebSocketClient(WebSocketClient):
...
class WebsocketEngine(object):
    """
    WebsocketEngine connects remotely to a Websocket server and allow to send and receive files.
    """

    def __init__(self, ws):
        """
        Constructs a WebsocketEngine"""
        self.ws = ws
        self.url = ws.url
...
def create_and_connect_to_websocket(url):
    ...
    ws = ThreadedWebSocketClient(url)
