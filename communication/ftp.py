    ...
    def _remove_dir_tree_recursively(self, remote_path):
        try:
            ...
            if e.__cause__ and str(e.__cause__).startswith("550"):
                return  # No such file or directory 鈥?nothing to delete
            raise
        self._connect()
...
def make_ftp_client(  # noqa: PLR0913 - explicit protocol connection settings
    ...
    try:
        ...
        if protocol == "ftps":
            session.prot_p()
        return FtpClient(session, hostname, username, password, port, protocol)
    except Exception:
        session.close()
        raise
